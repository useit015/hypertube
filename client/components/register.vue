            ...
            <v-layout column justify-center>
                ...
                <v-form ref="form" v-model="valid" lazy-validation class="form">
                    ...
                    <v-layout column justify-center align-center class="mt-5 py-4">
                        <v-btn class="cta_btn" rounded large outlined color="primary" @click="registerUser">{{
                            $t('register')
                        }}</v-btn>
                    </v-layout>
                </v-form>
    ...
    methods: {
        ...
        getBase64: file =>
            new Promise((resolve, reject) => {
                ...
                if (['image/jpeg', 'image/png'].includes(file.type) == false) resolve(this.$t('rules.avatar.ext'))
                if (file.size >= 1024 * 1024 * 4) resolve(this.$t('rules.avatar.size'))
        ...
        async registerUser() {
            if (this.$refs.form.validate()) {
                try {
                    ...
                    let txt
                    if (!!res.data.err) {
                        txt = res.data.errors.map(cur => this.$t(`api.register.${cur}`)).join(', ')
