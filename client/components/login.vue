            ...
            <v-layout column justify-center>
                ...
                <v-form ref="form" v-model="valid" lazy-validation class="mt-5">
                    ...
                    <v-layout justify-end align-center>
                        <v-btn rounded text small color="primary" class="caption fgt_btn" to="/sign/forgot" nuxt>{{
                            $t('forgot')
                        }}</v-btn>
                    </v-layout>
                    <v-layout justify-center align-center class="mt-5 py-4">
                        <v-btn class="cta_btn" rounded large outlined color="primary" @click.prevent="logUser">{{
                            $t('login')
                        }}</v-btn>
                    </v-layout>
                </v-form>
        ...
        async logUser() {
            if (this.$refs.form.validate()) {
                try {
                    ...
                    if (!!res.data.err) {
                        const txt = res.data.errors.map(cur => this.$t(`api.login.${cur}`)).join(', ')
