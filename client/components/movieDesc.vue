<template>
    <v-overlay :value="open" color="black" opacity=".9">
                ...
                <v-list dense>
                    <v-list-item-group color="primary">
                        <v-list-item v-for="(item, i) in movie.items" :key="i">
                            ...
                            <v-list-item-content>
                                <!-- <nuxt-link :to="`/watchtest/${item.id}`">{{ item.file }}</nuxt-link> -->
        ...
        <v-row v-if="trailer" column align="center" justify="center" class="trailer_overlay">
            <v-btn fab top right small outlined absolute color="primary" class="trailer_close" @click="trailer = false">
                <v-icon>close</v-icon>
            </v-btn>
            <youtube fitParent :player-vars="vars" ref="youtube" :video-id="movie.trailer" />
        </v-row>
        <v-btn fab top right small outlined absolute color="primary" class="movie_close" @click="open = false">
            <v-icon>close</v-icon>
        </v-btn>
    </v-overlay>
</template>
